...
def personalizar_desde_color(request, codigo_color):
    usuario = _get_usuario(request)
    if not usuario:
        messages.error(request, "Inicia sesión para personalizar.")
        return redirect('login_usuario')

    # 1. Reconstruimos el color con el # (porque en la URL viene limpio: 'FF0088')
    color_hex = f"#{codigo_color}"
    
    # 2. Obtenemos todas las pinturas base disponibles
    pinturas = Pintura.objects.all()

    if request.method == 'POST':
        # El usuario eligió una pintura del listado
        pintura_id = request.POST.get('pintura_id')
        cantidad = int(request.POST.get('cantidad', 1))
        
        pintura_elegida = get_object_or_404(Pintura, id=pintura_id)

        # 3. Crear el objeto Personalizado
        personalizado = Personalizado.objects.create(
            pintura=pintura_elegida,
            color=color_hex, # Usamos el color que vino de la URL
            usuario=usuario
        )
        ...
        messages.success(request, f"¡{pintura_elegida.nombre} color {color_hex} agregada al carrito!")
        return redirect('ver_carrito')

    return render(request, 'colormania/user/personalizar_desde_color.html', {
        'color_hex': color_hex,
        'codigo_limpio': codigo_color, # Por si lo necesitas
        'pinturas': pinturas
    })